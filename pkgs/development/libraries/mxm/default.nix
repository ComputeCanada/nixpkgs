{ stdenv, fetchurl, rpmextract, patchelf, libibverbs, zlib }:
...
stdenv.mkDerivation rec {
  ...
  buildInputs = [ rpmextract patchelf libibverbs zlib ];
  ...
  installPhase = ''
    ...
    for i in $out/bin/*; do
      ...
      patchelf --set-rpath $out/lib:${libibverbs}/lib:${zlib}/lib $i
    done
    ...
    for i in $out/lib/*.so; do
      patchelf --set-rpath ${libibverbs}/lib:${zlib}/lib $i
    done
